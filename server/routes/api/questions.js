...
router.get('/', (req, res) => {
  ...
  Questions.forEach((e, id) => {
    var questionObj = {}
    questionObj.question = e.question
    questionObj.options = e.options
    questionObj.id = id.toString()
    qs.push(questionObj)
  })
  res.status(200).send(
    qs
...
router.get('/:qId', (req, res) => {
  // Remove the lines below and write your implementation
  var question = Questions[req.params.qId]
  res.status(200).send({
    question : question.question,
    options : question.options,
    id: req.params.qId.toString()
  }) 
})
...
router.post('/result', (req, res) => {
  ...
  var summary
  var score = 0
  var total = Object.keys(reqBody).length
  Object.keys(reqBody).forEach((questionId) => {
    const userAns = reqBody[questionId]
    if (Questions[parseInt(questionId)].answer === userAns) {
      score++
    }
  })
  if (score >= 3) {
